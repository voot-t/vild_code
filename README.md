...
Important arguments of vild_main.py are as follows:
...
Demonstration datasets are included in directory imitation_data. 
* Directory imitation_data/TRAJ_h5 contains datasets for experiments except the Robosuite experiment. 
* Original dmonstrations for robosuite experiments can be obtained from the Roboturk project: https://roboturk.stanford.edu/dataset_sim.html (Mandlekar et al., 2018).
We include processed demonstrations used in our RobosuiteReacher experiment in directory imitation_data/TRAJ_robo, where we terminate original demonstrations when the end-effector touch the object and choose 10 demonstrations whose length are approximatedly 500 time steps. 
