    ...
    for ep_i in range(1, len(demos) + 1):
        ...
        for t in range (0, len(states)):
            ...
            if args.robo_task == "full":
                ...
                # reward = r_reach # this is inverse proportion to distance to object. So small distance -> high reward. 
                ...
                # done = env._check_contact() 
                # if done:
                #     print("step %d: contact. r_reach %f" % (t, r_reach) )

            elif args.robo_task == "grasp":
                r_reach, r_grasp, r_lift, r_hover = env.staged_rewards()
                reward = r_grasp    #does not matter, unless robo_reward_coef > 0
                # if gripper grasp object, task successes.
                if r_grasp > 0:
                    done = 1
        ...
        """ save data """        
        # traj_filename = traj_path + "/%s_reach_demo%d.p" % (env_name, ep_i)
        if args.robo_task != "full":
            traj_filename = traj_path + "/%s_%s_demo%d.p" % (env_name, args.robo_task, ep_i)
        else:
            traj_filename = traj_path + "/%s_demo%d.p" % (env_name, ep_i)
